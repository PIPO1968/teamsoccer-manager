...
const TeamLineups = () => {
  ...
  const handleZoneSelect = (zone: number) => {
    ...
    // Estado temporal para la alineación cruda
    const [rawLineup, setRawLineup] = useState<any>(null);

    // Primer efecto: obtener la alineación del backend
    useEffect(() => {
      let cancelled = false;
      const fetchLineup = async () => {
        ...
        try {
          ...
          if (!cancelled) {
            setRawLineup(data.success && data.lineup ? data.lineup : null);
          }
        } catch (error) {
          if (!cancelled) {
            setRawLineup(null);
    ...
    // Segundo efecto: mapear alineación a jugadores solo cuando players esté listo
    useEffect(() => {
      if (!rawLineup || isLoading || !players || players.length === 0) return;
      // Mapear posiciones
      const newPositions: { [key: number]: any } = {};
      if (rawLineup.positions && Array.isArray(rawLineup.positions)) {
        for (const pos of rawLineup.positions) {
      ...
      if (rawLineup.substitutes && Array.isArray(rawLineup.substitutes)) {
        for (const sub of rawLineup.substitutes) {
      ...
      setIsSaved(true);
      setIsDefaultSaved(rawLineup.is_default || false);
    }, [rawLineup, players, isLoading]);
