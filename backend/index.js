...
app.get('/auth/me', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            `SELECT user_id, username, email, is_admin, is_premium, premium_expires_at, status, country_id FROM managers WHERE user_id = $1`,
            [decoded.user_id]
        );
        if (!result.rows[0]) return res.json({ success: true, manager: null });
        res.json({ success: true, manager: result.rows[0] });
    } catch {
        res.json({ success: true, manager: null });
    }
});

// Obtener temporada y semana actual (real de la base de datos, o fallback)
app.get('/meta/current-season', async (req, res) => {
    try {
        // Buscar en tabla seasons si existe, si no, fallback
        let season = 1, week = 1;
        try {
            const result = await pool.query('SELECT current_season, current_week FROM seasons ORDER BY id DESC LIMIT 1');
            if (result.rows[0]) {
                season = result.rows[0].current_season;
                week = result.rows[0].current_week;
            }
        } catch {}
        res.json({ success: true, season: { current_season: season, current_week: week } });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// Estadísticas globales del mundo
app.get('/world/stats', async (req, res) => {
    try {
        // Total regiones
        const regionsResult = await pool.query('SELECT COUNT(*) AS total FROM leagues_regions');
        // Total managers
        const managersResult = await pool.query('SELECT COUNT(*) AS total FROM managers');
        // Managers en espera (status = waitlist)
        const waitingResult = await pool.query("SELECT COUNT(*) AS total FROM managers WHERE status = 'waitlist'");
        // Managers online
        const onlineResult = await pool.query('SELECT COUNT(*) AS total FROM managers WHERE is_online = true');
        // Total equipos
        const teamsResult = await pool.query('SELECT COUNT(*) AS total FROM teams');
        // Total ligas (series)
        const leaguesResult = await pool.query('SELECT COUNT(*) AS total FROM series');
        // Listado de ligas por región
        const leaguesList = await pool.query(
            `SELECT s.series_id AS league_id, r.region_id, r.name AS region_name,
                    (SELECT COUNT(*) FROM teams t WHERE t.series_id = s.series_id) AS team_count
             FROM series s
             LEFT JOIN leagues_regions r ON r.region_id = s.region_id
             ORDER BY r.region_id, s.series_id`
        );
        res.json({
            success: true,
            stats: {
                totalRegions: parseInt(regionsResult.rows[0]?.total || '0', 10),
                totalManagers: parseInt(managersResult.rows[0]?.total || '0', 10),
                totalWaitingManagers: parseInt(waitingResult.rows[0]?.total || '0', 10),
                onlineManagers: parseInt(onlineResult.rows[0]?.total || '0', 10),
                totalTeams: parseInt(teamsResult.rows[0]?.total || '0', 10),
                totalLeagues: parseInt(leaguesResult.rows[0]?.total || '0', 10),
                leagues: leaguesList.rows
            }
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/teams/:id/matches', async (req, res) => {
    ...
    try {
        ...
        const matches = result.rows.map(row => {
            ...
            // Forzar match_date a formato exacto ISO 8601 (YYYY-MM-DDTHH:mm:ssZ)
            let match_date = null;
            if (row.match_date) {
                try {
                    // Si ya es string y contiene 'T' y 'Z', dejarlo tal cual
                    if (typeof row.match_date === 'string' && row.match_date.includes('T') && row.match_date.endsWith('Z')) {
                        match_date = row.match_date;
                    } else {
                        // Si es string tipo '2026-09-19 19:15:00+00', convertir a ISO
...
app.post('/admin/players', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO players (
                first_name,last_name,position,age,nationality_id,team_id,
                value,wage,rating,pace,finishing,passing,defense,dribbling,heading,stamina,goalkeeper,crosses,ball_control,creativity,
            ...
            [
                ...
                p.crosses || 30, p.ball_control || 30, p.creativity || 30,
